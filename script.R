...
################# Supplementary Figure 3 (linear vs circular in hippo and cortex)
...
lin_all_hippo <- readRDS("./data/linearJunctions_hippo.rds")
...
lin_all_cortex <- readRDS("./data/linearJunctions_cortex.rds")
...
######################################## Figure 3 D and F (microglia)



covariate_m <-read.table("./data/covariate_file_microglia.txt", sep='\t', header=T) %>% tbl_df() %>% mutate(filename=sub("mapping/feature_counts/","", filename))%>% mutate(filename=sub(".all_mates.*","", filename))


aln_stats_mg <- read.table("./data/mapping_stats_microglia.tsv", sep='\t', header=T)%>% tibble::as_tibble() %>% left_join(covariate_m, by=c("sample_id"="filename")) %>% mutate(sample=label) %>% dplyr::select(sample, key, value)
...
microglia_circ_hippo <- readRDS("./data/microglia_hippocampus_circ_counts.rds")
...
microglia_circ_cortex <- readRDS("./data/microglia_cortex_circ_counts.rds")
...
microglia_circ <- readRDS("./data/microglia_circ_counts.rds")
...
pdf("microglia_Figure.pdf", height=6, width=9)
...
fly_samples<-read.table("./data/fly_samples.tsv")
...
total_reads<-read.table("./data/fly_reads_total.csv")
...
circ_all_fly <- readRDS("./data/circ_all_fly_counts.rds")
...
## normalized (reproducible) circRNA counts
...
### linear spliced junction reads

lin_all_fly <- readRDS("./data/linear_all_fly_counts.rds")
...
pdf("Fly_figure.pdf", height=13, width=9)
